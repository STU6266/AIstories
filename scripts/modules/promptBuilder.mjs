// promptBuilder.mjs
// Aufgabe: Aus den User-Einstellungen + optionalen Zufallseinschüben (randomSettings)
// einen klaren, gut strukturierten Start-Prompt erzeugen.
// Der Prompt beschreibt Tonalität, Themen, Länge (Kapitel/Minuten) und das Interaktionsformat.

export function buildFinalPrompt(userSettings, randomSettings) {
  // Defensive Defaults: Falls etwas fehlt, nie crasht es → leere Objekte statt null/undefined
  if (!userSettings || typeof userSettings !== 'object') userSettings = {};
  if (!randomSettings || typeof randomSettings !== 'object') randomSettings = {};

  // "extraText" sammelt optionale Bausteine (Rätsel, Item-Wahl, seltenes Bad Ending)
  let extraText = '';

  // PUZZLE-Einschübe: Liste → pro Eintrag "PUZZLE #i: <Text>"
  ...
  // ITEM-CHOICE-Einschübe: Liste → "ITEM CHOICE #i: <Text>"
  ...
  // seltener „Bad Ending“-Trigger (Text wird nur manchmal gesetzt)
  ...
  // - Zielgruppe (Alter), Thema, "Skalenwerte" für Tonalität,
  // - Struktur: Kapitel mit Choices am Ende,
  // - Gesamtlänge grob anhand der vom User gewünschten Minuten,
  // - finale Instruktion: "Beginne jetzt mit dem ersten Kapitel."
  const prompt =
    ...
    // Gute Final-Kapitellogik: die bisherigen Choices berücksichtigen und Stränge auflösen
    ...
    // Optional: Zusatzblöcke (Rätsel/Items/Bad Ending) sauber abtrennen
    (extraText ? ('\n\n---\n\n' + extraText + '\n') : '') +
    `\nBegin with the first chapter now.`;

  // Defensive Rückgabe: Falls etwas schief geht, gib eine generische Instruktion zurück
